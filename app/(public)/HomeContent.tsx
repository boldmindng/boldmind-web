...
import { getColorScheme } from "@boldmindng/utils/constants/colors";
...
// The one deliberate exception on this page: the Operator Zone section (below)
// borrows VillageCircle's own primary as a "chapter break" accent. Named once
// here instead of repeated as a bare literal, and explained again at the
// section itself.
const VILLAGECIRCLE_ACCENT = getColorScheme("villagecircle").primary;
const VILLAGECIRCLE_MUTED = getColorScheme("villagecircle").muted;

// ─── Pillar data (mirrors PILLAR_METADATA from products.ts) ─────────────────
// Colors now come from `getColorScheme(slug)` instead of duplicated literal
// hex — previously `#065F46` / `#3B1F0A` / `#1E40AF` / `#5B21B6` were retyped
// here by hand, which drifts silently the moment colors.ts changes.
const PILLAR_DATA = [
  {
    slug: "amebogist",
    Icon: Newspaper,
...
const PILLARS = PILLAR_DATA.map((p) => ({
  ...p,
  color: getColorScheme(p.slug).primary,
}));
...
export default function HomeContent() {
  ...
  // Small helper so every `whileHover`/`whileInView` on this page collapses
  // to a no-op transform (not just a shorter one) when the visitor has asked
  // for reduced motion — matches the quality floor in the design skill.
  const lift = (y: number) => (prefersReducedMotion ? undefined : { y });
  ...
  return (
    ...
    <div style={{ backgroundColor: brand.bg, overflowX: "hidden" }}>
      ...
      <section
        ...
        style={{
          background: `linear-gradient(135deg, var(--product-primary) 0%, color-mix(in srgb, var(--product-primary) 55%, black) 100%)`,
        }}
      >
        {/* Decorative blobs */}
        <div
          aria-hidden="true"
          className="absolute top-0 right-0 w-125 h-125 rounded-full blur-[120px] pointer-events-none"
          style={{ backgroundColor: brand.secondary, opacity: 0.07 }}
        />
        <div
          aria-hidden="true"
          className="absolute bottom-0 left-0 w-100 h-100 rounded-full blur-[100px] pointer-events-none"
        ...
        <div className="relative max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          >
            ...
            >
              One ecosystem. Four pillars. From stranger to builder — we take
              Nigerians the full distance.{" "}
              ...
              >
                <div className="flex items-start justify-between mb-3">
                  ...
                  <span
                    ...
                    style={{
                      backgroundColor: `${pillar.color}35`,
                      color: pillar.color,
                    }}
                  >
                    {pillar.step}
                ...
                <h3
                  ...
                  style={{ color: "white", marginBottom: "0.125rem" }}
                >
                  {pillar.name}
                </h3>
                <p
      ...
      <section className="py-16 sm:py-24" style={{ backgroundColor: brand.bg }}>
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-14">
            ...
            >
              The{" "}
              <span style={{ color: brand.primary }}>BoldMind Flywheel</span>
            </h2>
              ...
              >
                ...
                <div className="relative z-10">
                  <div className="relative inline-flex mb-4">
                    ...
                    <span
                      className="absolute -top-2 -right-2 w-6 h-6 rounded-full text-[11px] font-black flex items-center justify-center text-white shadow tabular-nums"
      ...
      {/* ─────────────────────────────────────────── OPERATOR ZONE */}
      {/*
        Background intentionally uses VillageCircle's muted token value.
        ...
        now pulled from getColorScheme('villagecircle') rather than a repeated literal.
      */}
      <section
        className="py-16 sm:py-24"
        style={{ backgroundColor: VILLAGECIRCLE_MUTED }}
      >
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-center">
            ...
            <div>
              <p
                className="text-[11px] font-black uppercase tracking-widest mb-3"
                style={{ color: VILLAGECIRCLE_ACCENT }}
              >
                The Operator Zone
              </p>
              <h2
              ...
              >
                Your BoldMind account is the cockpit for everything. PlanAI
                subscriptions, EduCenter progress, Vibe Coders cohort status,
                and cross-pillar upsells — all in one dashboard. No re-logging.
                No friction.
              </p>
              ...
              */}
              ...
              >
                ...
                <div className="p-5 space-y-3">
                  ...
                  >
                    <div
                      className="flex items-center gap-1.5 text-[10px] font-black uppercase tracking-widest mb-1"
                      style={{ color: brand.secondary }}
                    >
                  ...
                  <div
                    className="rounded-xl p-4 border-l-4"
                    style={{
                      backgroundColor: "rgba(43,77,135,0.063)",
                  ...
                  >
                    ...
                    >
                      <Zap className="w-3 h-3" aria-hidden="true" /> Cross-sell
                    </div>
                    <div
      ...
      <section
        className="py-16 sm:py-24"
        style={{
          background: `linear-gradient(135deg, var(--product-primary) 0%, color-mix(in srgb, var(--product-primary) 55%, black) 60%, color-mix(in srgb, var(--product-primary) 30%, black) 100%)`,
        }}
      >
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          >
            ...
            <p
              className="text-sm mb-10 tabular-nums"
              style={{
                color: "rgba(255,255,255,0.45)",
                marginBottom: "2.5rem",
              }}
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center mb-8">
              ...
              >
                {/* Official WhatsApp brand mark — a real logo, not a stand-in
                    icon, so this is the correct pattern rather than an emoji. */}
                <svg
            ...
            <p className="text-xs" style={{ color: "rgba(255,255,255,0.35)" }}>
              NDPA Compliant · No card required · Paystack Payments · Built in
              Nigeria
            </p>
          </motion.div>
        </div>
