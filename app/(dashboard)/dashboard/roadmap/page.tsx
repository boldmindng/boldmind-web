...
function StatCard({
  ...
  const palette: Record<string, string> = {
    ...
    green:
      "bg-green-50 dark:bg-green-950/20 border-green-200 dark:border-green-800/40 text-green-600 dark:text-green-400",
    emerald:
    ...
    >
      <div className="flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3 shrink-0">
          ...
          <span
            className={`px-3 py-1 rounded-full text-xs font-semibold ${STATUS_BADGE[item.status]}`}
          >
            {item.status}
          </span>
        </div>
...
export default function RoadmapDashboard() {
  const byQuarter = MILESTONES.reduce<Record<string, Milestone[]>>(
    (acc, m) => {
      const quarterMilestones = acc[m.quarter] ?? [];
      quarterMilestones.push(m);
      acc[m.quarter] = quarterMilestones;
      return acc;
    },
          ...
          <div className="max-w-4xl mx-auto space-y-10">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-5">
              <StatCard
                icon={Target}
                label="Total Planned"
                value={String(SUMMARY.totalPlanned)}
                color="blue"
              />
              <StatCard
                icon={Calendar}
                label="Q2 2026"
                value={String(SUMMARY.q2Launches)}
                color="amber"
              />
              <StatCard
                icon={Clock}
            ...
            <p className="text-xs text-gray-300 dark:text-gray-600 text-center pb-4">
              Data is static until <code>/hub/roadmap</code> endpoint is
              registered in NestJS. Add <code>getRoadmap()</code> to{" "}
              <code>hub-api-adapter.ts</code> to make it live.
            </p>
          </div>
        </main>
      </div>
