...
export default function ProductsPage() {
  ...
  const handleUpgrade = async (productSlug: string) => {
    try {
      ...
      const authUrl = (res as any).data.authorizationUrl;
  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        >
          All products in the BoldmindNG ecosystem. One account unlocks
          everything.
        </p>
      </div>
      ...
      {!loading && subs.length > 0 && (
        <div>
          ...
          <div className="space-y-3">
            {subs.map((sub: any) => {
              const product = BOLDMIND_PRODUCTS.find(
                (p) => p.slug === sub.productSlug,
              );
              return (
                <div
                  key={sub.productSlug}
