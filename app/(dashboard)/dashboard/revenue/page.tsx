...
function StatCard({
  ...
  const palette: Record<string, string> = {
    green:   'bg-green-50 dark:bg-green-950/20 border-green-200 dark:border-green-800/40',
    emerald: 'bg-emerald-50 dark:bg-emerald-950/20 border-emerald-200 dark:border-emerald-800/40',
    blue:    'bg-blue-50 dark:bg-blue-950/20 border-blue-200 dark:border-blue-800/40',
    amber:   'bg-amber-50 dark:bg-amber-950/20 border-amber-200 dark:border-amber-800/40',
  };
  ...
  return (
    <div className="space-y-1.5">
      <div className="flex items-center justify-between gap-3">
        <p className="text-sm font-medium text-gray-800 dark:text-gray-200 truncate flex-1">{name}</p>
        <div className="text-right flex-shrink-0">
          <p className="text-sm font-bold text-gray-900 dark:text-white">₦{revenue.toLocaleString()}</p>
          ...
          <div className="max-w-5xl mx-auto space-y-10">
            ...
            {error && (
              <div className="bg-red-50 border border-red-200 rounded-xl px-4 py-3 flex items-center justify-between">
                ...
                <button onClick={load} className="text-red-600 text-sm font-medium hover:text-red-800">Retry</button>
              </div>
            )}

            {/* KPI cards */}
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
              <StatCard icon={DollarSign}  label="Monthly Revenue" loading={loading}
                value={`₦${totalRevenue.toLocaleString()}`} color="green" />
              <StatCard icon={TrendingUp}  label="MoM Growth"      loading={loading}
                value={growthMoM} color="emerald" />
              <StatCard icon={ArrowUpRight} label="ARR"            loading={loading}
                value={`₦${arr.toLocaleString()}`}
                sub="Annual Run Rate" color="blue" />
            </div>
            ...
            <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 shadow-lg p-6">
              ...
              {!loading && topProducts.length > 0 && (
                <div className="mt-6 pt-5 border-t border-gray-100 dark:border-gray-800 flex items-center justify-between">
                  <p className="text-sm font-semibold text-gray-600 dark:text-gray-400">Total MRR</p>
