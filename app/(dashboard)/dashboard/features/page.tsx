...
function FeatureCard({
  ...
  return (
    ...
    >
      ...
      />
      <h3 className="font-bold text-base mb-1" style={{ color: isActive ? 'white' : 'inherit' }}>
        {feature.title}
      </h3>
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 mb-1">
        ...
        <span className="font-medium text-sm text-gray-800 dark:text-gray-200">{label}</span>
      </div>
...
export default function FeaturesPage() {
  ...
  const productCount   = Array.isArray(products)        ? products.length        : (products as any)?.length ?? '—';
  ...
  const targetAchievement = topPriority?.length
    ? `${Math.round((topPriority.filter(p => p.status === 'active').length / topPriority.length) * 100)}%`
    : '—';
    ...
    <div className="flex-1 p-6 lg:p-10 overflow-auto">
      <div className="max-w-7xl mx-auto space-y-12">
          ...
          {activeFeature === 'dashboard' && (
            <div className="border-t pt-8">
              <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">
                ...
                {productsLoading && <Loader2 size={14} className="inline ml-2 animate-spin text-gray-400" />}
              </h3>
              {productsLoading ? (
                ...
                <p className="text-sm text-gray-400 italic">No products returned from API.</p>
              )}
            </div>
          )}
          ...
          {activeFeature === 'revenue' && (
            <div className="border-t pt-8">
              ...
              {statsLoading ? (
                ...
                <div className="space-y-2">
                  {topPriority.slice(0, 6).map((p, i) => (
                    <div key={i} className="flex items-center gap-4 px-4 py-3 rounded-xl border border-gray-100 dark:border-gray-800">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-semibold truncate text-gray-800 dark:text-white">{p.name}</p>
                ...
                <p className="text-sm text-gray-400 italic">No revenue data available.</p>
              )}
            </div>
          )}
        </motion.div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 pb-6">
          <QuickStat icon={Globe}       label="Products"          value={String(productCount)}    color="#3B82F6" loading={productsLoading} />
          <QuickStat icon={DollarSign}  label="Monthly Revenue"   value={monthlyRevenue}          color="#22C55E" loading={statsLoading}    />
          <QuickStat icon={Users}       label="Team Members"      value={String(teamSize)}        color="#A855F7" loading={statsLoading}    />
          <QuickStat icon={Target}      label="Target Achievement" value={targetAchievement}      color="#F97316" loading={statsLoading}    />
        </div>
      </div>
    </div>
  );
