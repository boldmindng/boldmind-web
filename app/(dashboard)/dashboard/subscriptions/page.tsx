...
function SubscriptionCard({ sub }: { sub: Subscription }) {
  ...
  return (
    <div className={`rounded-2xl border-2 p-5 transition-all ${
      isActive ? 'border-green-200 bg-green-50/50 dark:bg-green-950/10 dark:border-green-800/50' : 'border-gray-200 bg-white dark:bg-gray-900 dark:border-gray-800'
    }`}>
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-center gap-3">
          <div className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 ${
            ...
            <Package2 size={18} className={isActive ? 'text-green-600' : 'text-gray-400'} />
          </div>
          <div>
        ...
        <div className="space-y-0.5">
          {sub.amount != null && (
            <p className="text-sm font-semibold text-gray-700 dark:text-gray-300">
              ₦{sub.amount.toLocaleString()} / {sub.plan === 'annual' ? 'year' : 'month'}
            </p>
          )}
          {expiryStr && (
            <p className="text-xs text-gray-400">
              {isActive ? 'Renews' : 'Expired'} {new Date(expiryStr).toLocaleDateString('en-NG', { day: 'numeric', month: 'short', year: 'numeric' })}
            </p>
          )}
        </div>
        {!isActive && (
          <a href={`/pricing#${sub.productSlug}`}
...
function UpgradeCard({
  ...
  return (
    <div className="rounded-2xl border-2 border-dashed border-gray-200 dark:border-gray-700 p-5 flex flex-col gap-4 hover:border-blue-300 transition-colors">
      ...
      >
        {loading ? <Loader2 size={14} className="animate-spin" /> : <><Zap size={14} /> Upgrade</>}
      </button>
    </div>
  );
}
    ...
    <div className="max-w-3xl mx-auto py-6 px-4 sm:px-0 space-y-8">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-400 text-sm mt-0.5">Manage your active plans and payments</p>
        </div>
      ...
      {error && (
        <div className="flex items-center justify-between bg-red-50 border border-red-200 rounded-xl px-4 py-3">
          ...
          <button onClick={refresh} className="text-red-600 text-sm font-medium hover:text-red-800">Retry</button>
        </div>
      )}
      ...
      {!loading && expiredSubs.length > 0 && (
        <section>
          <h2 className="text-base font-bold text-gray-500 dark:text-gray-400 mb-3">Expired Plans</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
      ...
      <section>
        ...
        <p className="text-xs text-gray-400 mt-3 text-center">
          <a href="/pricing" className="hover:text-blue-600 underline">View all products and pricing →</a>
        </p>
      </section>
    </div>
  );
