...
function SkeletonCard() {
  return (
    <div className="rounded-2xl p-5 border-2 animate-pulse" style={{ backgroundColor: 'var(--product-background)', borderColor: 'var(--product-muted)' }}>
      <div className="flex items-center gap-3 mb-3">
...
export default function HubDashboardPage() {
  ...
  const displayName = (user as any)?.firstName || user?.name?.split(' ')[0] || user?.email?.split('@')[0] || 'Founder';
  ...
  if (error) {
    return (
      <div className="flex flex-col items-center justify-center h-64 gap-4">
        ...
        <button onClick={refresh} className="px-4 py-2 rounded-xl text-sm font-semibold text-white" style={{ backgroundColor: 'var(--product-primary)' }}>
          Retry
        </button>
      </div>
    );
  ...
  return (
    <div className="space-y-8">
      ...
      >
        <div className="absolute top-0 right-0 w-48 h-48 opacity-10 rounded-full blur-3xl" style={{ backgroundColor: 'var(--product-secondary)', transform: 'translate(30%,-30%)' }} />
        <h1 className="text-2xl font-black text-white mb-1">Welcome back, {displayName} 🚀</h1>
        <p className="text-white/70 text-sm">BoldmindNG Ecosystem — 32+ products, one hub.</p>
        {growth && <div className="mt-3"><GrowthBadge trend={growth.trend} percentage={growth.percentage} /></div>}
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
        {loading ? (
          ...
          <>
            <StatCard icon={UsersIcon}   label="Total Users"       value={totalUsers.toLocaleString()} sub={`${stats?.userStats?.growth?.currentMonth ?? 0} this month`} />
            <StatCard icon={PackageIcon} label="Active Products"    value={activeProducts} />
            <StatCard icon={ShieldIcon}  label="Admins"             value={totalAdmins} />
            <StatCard icon={DollarIcon}  label="Monthly Revenue"    value={`₦${monthlyRevenue.toLocaleString()}`} sub={`Team: ${stats?.ecosystemOverview?.totalTeamSize ?? 0}`} accent />
          </>
        )}
      </div>
      ...
      {/* ── Quick actions ────────────────────────────────────────────── */}
      <div>
        <h2 className="text-lg font-black mb-4" style={{ color: 'var(--product-foreground)' }}>Quick Actions</h2>
      ...
      {!loading && (stats?.userStats?.topProducts?.length ?? 0) > 0 && (
        <div>
          <h2 className="text-lg font-black mb-4" style={{ color: 'var(--product-foreground)' }}>Top Products by Users</h2>
          <div className="rounded-2xl border-2 overflow-hidden" style={{ borderColor: 'var(--product-muted)', backgroundColor: 'var(--product-background)' }}>
            {stats!.userStats.topProducts.map((product, i) => (
              ...
              >
                <span className="w-6 h-6 rounded-full flex items-center justify-center text-xs font-black text-white flex-shrink-0" style={{ backgroundColor: 'var(--product-primary)' }}>{i + 1}</span>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-semibold truncate" style={{ color: 'var(--product-foreground)' }}>{product.productName}</p>
                  <p className="text-xs" style={{ color: 'var(--product-foreground)', opacity: 0.4 }}>{product.productSlug}</p>
                </div>
                <span className="text-sm font-black" style={{ color: 'var(--product-primary)' }}>{product.userCount.toLocaleString()} users</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-black" style={{ color: 'var(--product-foreground)' }}>Recent Activity</h2>
          <Link href="/admin/logs" className="text-sm font-bold" style={{ color: 'var(--product-primary)' }}>View all →</Link>
        </div>
        <div className="rounded-2xl border-2 overflow-hidden" style={{ borderColor: 'var(--product-muted)', backgroundColor: 'var(--product-background)' }}>
          {loading ? (
            Array.from({ length: 5 }).map((_, i) => <SkeletonRow key={i} />)
          ) : (stats?.recentActivity?.length ?? 0) === 0 ? (
            <div className="px-5 py-8 text-center text-sm" style={{ color: 'var(--product-foreground)', opacity: 0.4 }}>No recent activity</div>
          ) : (
      ...
      {!loading && (stats?.systemHealth?.length ?? 0) > 0 && (
        <div>
          <h2 className="text-lg font-black mb-4" style={{ color: 'var(--product-foreground)' }}>System Health</h2>
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
            {stats!.systemHealth.map(service => (
              <div key={service.name} className="rounded-xl p-4 border-2" style={{ borderColor: 'var(--product-muted)', backgroundColor: 'var(--product-background)' }}>
                <div className="flex items-center gap-2 mb-1">
                  <span className={`w-2 h-2 rounded-full flex-shrink-0 ${service.status === 'healthy' ? 'bg-green-400' : service.status === 'unhealthy' ? 'bg-red-400' : 'bg-yellow-400'}`} />
                  <p className="text-sm font-semibold truncate" style={{ color: 'var(--product-foreground)' }}>{service.name}</p>
                </div>
                {service.responseTime != null && (
                  <p className="text-xs" style={{ color: 'var(--product-foreground)', opacity: 0.4 }}>{service.responseTime}ms</p>
                )}
              </div>
            ))}
          </div>
...
const QUICK_LINKS = [
  ...
  { href: '/dashboard/referrals',emoji: '🔗', label: 'Referrals', sub: 'Track & share your link'    },
  { href: '/settings',           emoji: '⚙️', label: 'Settings',  sub: 'Profile & preferences'      },
] as const;
