...
import { broadcastLogout } from '@boldmindng/auth';
...
export default function ProtectedLayout({ children }: { children: ReactNode }) {
  ...
  const displayName = (user as any)?.firstName ?? user?.name ?? user?.email?.split('@')[0] ?? 'Builder';
  ...
  return (
    <div className="flex min-h-screen" style={{ backgroundColor: 'var(--product-background)' }}>
      ...
      >
        {/* Logo */}
        <div className="p-5 border-b" style={{ borderColor: 'var(--product-muted)' }}>
          <Link href="/dashboard" className="flex items-center gap-3">
            ...
            <div>
              <div className="font-black text-sm" style={{ color: 'var(--product-foreground)' }}>BoldmindNG</div>
              <div className="text-[10px] font-semibold uppercase tracking-widest" style={{ color: 'var(--product-secondary)' }}>Hub</div>
            </div>
          </Link>
        </div>

        {/* User pill */}
        <div className="p-4 border-b" style={{ borderColor: 'var(--product-muted)' }}>
          <div className="flex items-center gap-3 p-3 rounded-xl" style={{ backgroundColor: 'var(--product-highlight)' }}>
            <div
            ...
            <div className="min-w-0">
              <div className="text-sm font-bold truncate" style={{ color: 'var(--product-foreground)' }}>{displayName}</div>
              <div className="text-[11px] truncate" style={{ color: 'var(--product-foreground)', opacity: 0.5 }}>
                {user?.role === 'super_admin' ? 'Super Admin' : user?.role ?? 'Builder'}
              </div>
            </div>
          </div>
        </div>
        ...
        <nav className="flex-1 overflow-y-auto p-3 space-y-0.5">
          <p className="text-[10px] font-black uppercase tracking-widest px-3 py-2" style={{ color: 'var(--product-foreground)', opacity: 0.35 }}>
            Hub
          </p>
          {NAV_ITEMS.map(item => {
          ...
          <p className="text-[10px] font-black uppercase tracking-widest px-3 py-2 mt-4" style={{ color: 'var(--product-foreground)', opacity: 0.35 }}>
            Ecosystem
          </p>
          {ECOSYSTEM_LINKS.map(item => (
            ...
            >
              <span className="text-base">{item.icon}</span>
              {item.label}
              <span className="ml-auto text-[10px]" style={{ opacity: 0.4 }}>↗</span>
            </a>
          ))}
        </nav>

        {/* Sign out */}
        <div className="p-4 border-t" style={{ borderColor: 'var(--product-muted)' }}>
          <button
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        >
          {/* Mobile logo */}
          <Link href="/dashboard" className="lg:hidden font-black text-sm" style={{ color: 'var(--product-primary)' }}>
            BoldmindNG
          </Link>
          <div className="flex-1" />
          {/* Topbar actions */}
          <Link href="/dashboard/notifications" className="w-9 h-9 rounded-xl flex items-center justify-center text-lg" style={{ backgroundColor: 'var(--product-highlight)' }}>
            🔔
          </Link>
