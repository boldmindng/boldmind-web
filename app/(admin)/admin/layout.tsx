...
function AdminSidebarContent({
  ...
  const roleLabel = getUserRoleDisplay(user?.role ?? "guest");
  ...
  return (
    ...
    >
      ...
      {/* User footer */}
      <div
        className="shrink-0 border-t px-3 pb-3 pt-2 space-y-1"
        style={{ borderColor: "var(--product-muted)" }}
      >
        <div
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            ...
            {/* FIXED: was `user?.isSuperAdmin ? 'Super Admin' : user?.role` —
                isSuperAdmin doesn't exist on AuthUser (only role/ecosystemRole/
                permissions do). Now uses the centralized display resolver so
                every role, not just super_admin, gets a real label. */}
            <span
...
export default function AdminLayout({
  ...
  // FIXED: was `const { user } = useAuthStore();` — isLoading was read below
  // (in the useEffect deps and the loading-screen branch) without ever being
  // destructured. That's a ReferenceError at runtime / compile error under
  // strict mode, not a logic bug — nothing after this line could have worked.
  const { user, status, clearSession } = useAuthStore();
  const isLoading = status === "loading";

  // FIXED: was `user?.role === 'admin' || user?.role === 'super_admin'` —
  // duplicated the same role-gating logic that's now centralized (see
  // ProtectedLayout.tsx, AdminUsersPage.tsx). Also that version silently
  // excluded every other internal staff role (manager/editor/support/
  // analyst) from the admin panel entirely, whether or not that was intended.
  // cast to unknown first to satisfy TypeScript's stricter type checking.
  const isAdmin = ADMIN_ROLES.includes(
    (user?.role ?? "guest") as unknown as RoleDefinition,
  );
  ...
  useEffect(() => {
    // avoid unnecessary synchronous state update on every route change
  ...
  // FIXED: was `await logout();` — no `logout` function exists on the auth
  // store (confirmed across every other file that touches useAuthStore in
  // this codebase — the store only exposes state + clearSession/setSession).
  // Same sign-out sequence as ProtectedLayout.tsx: revoke server-side
  // sessions, broadcast to other tabs, clear local state.
    ...
    return (
      ...
      >
        <div
          ...
          style={{
            borderTopColor: "var(--product-primary)",
        ...
        >
          ...
          <div className="flex items-center gap-1.5">
            ...
            <div className="relative" ref={menuRef}>
              ...
              {userMenuOpen && (
                <div
                  className="absolute right-0 mt-1 w-52 rounded-2xl border shadow-xl z-50 overflow-hidden"
                  style={{
                    backgroundColor: "var(--product-background)",
                    borderColor: "var(--product-muted)",
                  }}
